...
class JobApplicationAgent:
    ...
    def match_skills_with_job(self) -> Dict:
        ...
        try:
            ...
            # Create a simplified prompt that will produce a smaller, more reliable response
            prompt = PromptTemplate(
                template="""Perform a focused analysis comparing the candidate's resume with the job description to identify matches and gaps.
                ...
                {{
                    "overall_match": {{
                        "percentage": 85,
                        "assessment": "Brief assessment of overall match (1-2 sentences)",
                    ...
                    "skill_match": {{
                        "matching_skills": [
                            {{
                                "skill": "skill name",
                                "resume_evidence": "brief evidence",
                        ...
                        "missing_skills": [
                            {{
                                ...
                                "acquisition_suggestion": "brief suggestion"
                            }}
                        ]
                    }},
                    "experience_match": {{
                        "match_assessment": "Brief assessment of experience match (1 sentence)",
                    ...
                    "education_match": {{
                        "match_level": "high/medium/low",
                        "assessment": "Brief assessment (1 sentence)"
                    }},
                    "strengths_for_role": [
                        {{
                            "strength": "strength",
                ...
                1. Return ONLY the JSON object, no other text
                2. ALL keys must be present in the response
                3. Keep all text fields BRIEF - no more than 1-2 sentences each
                4. Limit arrays to 5 items maximum
                5. Do not include any explanatory text or markdown
                """,
            ...
            resume_info_dict = {
                "personal_info": self.resume_analysis.get("personal_info", {}),
                "skills": self.resume_analysis.get("skills", {}),
                "experience": self.resume_analysis.get("experience", [])[:3],  # Limit to 3 experiences
                "education": self.resume_analysis.get("education", [])[:2]     # Limit to 2 education entries
            }
            resume_info = json.dumps(resume_info_dict, indent=2)
            
            # Simplify job analysis to reduce token usage
            job_analysis_dict = {
                "company_name": self.job_analysis.get("company_name", ""),
                "job_title": self.job_analysis.get("job_title", ""),
                "technical_skills": self.job_analysis.get("technical_skills", []),
                "soft_skills": self.job_analysis.get("soft_skills", []),
                "required_experience": self.job_analysis.get("required_experience", ""),
                "education_requirements": self.job_analysis.get("education_requirements", [])
            }
            job_analysis = json.dumps(job_analysis_dict, indent=2)
            ...
            try:
                ...
                # Add cultural fit if missing (to maintain compatibility with existing code)
                if "cultural_fit" not in self.skill_match_analysis:
                    self.skill_match_analysis["cultural_fit"] = {
                        "assessment": "Cultural fit assessment not available in simplified analysis.",
                        "matching_values": [],
                ...
                try:
                    ...
                    fixed_json = self._fix_truncated_json(response_text)
                    self.skill_match_analysis = json.loads(fixed_json)
                    logger.info("Successfully parsed skill match analysis after fixing JSON")
                    return self.skill_match_analysis
        ...
        try:
            ...
            if missing_skills:
                for skill in missing_skills[:5]:
                    ...
                    acquisition = skill.get("acquisition_suggestion", "").title()
                    summary += f"  • {skill_name}"
                    if importance:
                        summary += f" (Importance: {importance}"
                        if acquisition:
                            summary += f", Suggestion: {acquisition})"
                        else:
                            summary += ")"
                    summary += "\n"
